...
``` {r dplyr installation, message = FALSE}
...
``` {r ggplot2 installation, message = FALSE}
    pckglist <- rownames(installed.packages()) 
...
```
``` {r mean median ds print}
    dsmean
    dsmed
```

Hence, we determine that the mean number of total steps per day is
**`r format(dsmean, scientific = FALSE)`**, and  the median is 
**`r dsmed`**. Looking at the above histogram, this seems reasonable.
...
``` {r mean median ds imp}
    dsmean_imp <- mean(num_steps_imp$dailysteps)
    dsmed_imp <- median(num_steps_imp$dailysteps)
```
``` {r mean median ds imp print}
  dsmean_imp
  dsmed_imp
```

For the imputed data set, the mean number of total steps per day is
**`r format(dsmean_imp, scientific = FALSE)`**, and the median is 
**`r format(dsmed_imp, scientific = FALSE)`**. 
For the non-imputed data set, the mean number of total steps per day was
**`r format(dsmean, scientific = FALSE)`**, and the median was 
**`r format(dsmed, scientific = FALSE)`**. Therefore, while imputation did not
